...
function crear_EDI($codigo_orden) {
    ...
    if ($EDI_ESTADO === 0) {
        ...
        //$edi->agregar_segmento('LOC', '165+' . $EDI['loc165'] . ':139:6+' . $SENDER_ID); //EDI modificado para MSC, se retomara la configuracion previa
        $edi->agregar_segmento('LOC', '165+' . $EDI['loc165'] . $SENDER_ID. ':139:6');
        if ($IO == '2') // export
