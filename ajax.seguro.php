...
function run_part__obtener_info_bloque()
{
    ...
    LEFT JOIN `myr_orden_taller` AS ot 
     ON ot.`n_orden` = t1.`codigo_orden`
    LEFT JOIN `myr_cliente_taller` AS ct
     ON ct.`idCliente` = ot.`id_cliente`
    WHERE t1.`estado` = 'dentro' AND t3.x2='$x2' AND t3.y2='$y2'
    ...
    while ($f = db_fetch($r))
    {
        ...
        if ($f['tipo_contenedor'] == "TLL20") 
        {
        $title .= '<hr /><p>';
        $title .= '<b>'.$f['nombre'].'</b><br />';
        $title .= 'Cliente: <b>'.$f['nombreCliente'].'</b></br>';
        $title .= 'Placa: <b>'.$f['Placa'].'</b><br />';
        $title .= 'Kolometraje: <b>'.$f['kilometraje'].'</b><br />';
        $title .= 'Piloto: <b>'.$f['piloto'].'</b><br />';
        $title .= 'Fecha de Ingreso:<b>'.$f['f_ingreso'].'</b><br>';
         $title .= 'Observaciones de Ingreso: <b>'.$f['observaciones_ingreso'].'</b><br>';
        $title .= 'OT : <a href="util.orden.add.html?ID='.$f['codigo_orden'].'">'.$f['codigo_orden'].'</a>';
        $title .= '</p>';
        }
        else
        {
        ...
        $title .= 'Contenedor: <a href="#" rel="'.$f['codigo_contenedor'].'" class="ejecutar_busqueda_codigo_contenedor"><b>'.$f['codigo_contenedor'].'</b></a> <a  href="#" class="bq_usar_contenedor" col="'.$f['x2'].'" fila="'.$f['y2'].'" nivel="'.$f['nivel'].'">[ Utilizar posición en vista acual ]</a><br>';
        $title .= 'OT : <a href="util.orden.add.html?ID='.$f['codigo_orden'].'">'.$f['ot_numero'].'</a>';
        $title .= '</p>';
        }
    }
