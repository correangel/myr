...
WHERE t1.`estado` = 'dentro' AND t1.`codigo_patio` = $codigo_patio
";
        ...
        if (@is_array($mapa[$xy]['datos'])) {
            ...
            foreach($mapa[$xy]['datos'] as $kNivel => $vDatos)
            {
                ...
                if ($tipo == "taller") 
                {
                    ...
                    $qtip .= '<b>'.$mapa[$xy]['datos'][$kNivel]['nombre'].'</b> Equipo en reparaci&oacute;n</b><br />';
                    $qtip .= '<table class="tabla-estandar opsal_tabla_ancha opsal_tabla_borde_oscuro">';
                    $qtip .= '<tr><td>Cliente</td><td>'.$mapa[$xy]['datos'][$kNivel]['nombreCliente'].'</b></td></tr>';
                    $qtip .= '<tr><td>Placa</td><td>'.$mapa[$xy]['datos'][$kNivel]['Placa'].'</td></tr>';
                    $qtip .= '<tr><td>Kilometraje</td><td>'.$mapa[$xy]['datos'][$kNivel]['kilometraje'].'</td></tr>';
                }   
                else
                {
                        $qtip .= '<div style="padding-top:5px;">';
